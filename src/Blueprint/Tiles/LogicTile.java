...
public class LogicTile extends ExecTile {
	...
	public String parse() {
		...
		if(this.getLogicType() == LogicType.AND) {
			seg += " && ";
		}else if(this.getLogicType() == LogicType.OR) {
			seg += " || ";
		}else if(this.getLogicType() == LogicType.EQUAL) {
			seg += " == ";
		}else if(this.getLogicType() == LogicType.NOT_EQUAL) {
			seg += " != ";
		}
	...
	public void draw(Graphics g) {
		...
		if(combo.getItemCount() == 0) {
			for(LogicType mt : LogicType.values()) {
				combo.addItem(mt.toString());
			}
			combo.setSelectedItem(this.getLogicType().toString());
		}else {
			this.setLogicType(LogicType.valueOf(combo.getSelectedItem().toString()));
		}
